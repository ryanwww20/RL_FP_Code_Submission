...
Uses Stable-Baselines3 for A2C implementation
"""
...
class TrainingCallback(BaseCallback):
    ...
    Follows README structure: a2c_model_log_<start_time>/ with img/, plot/, result.csv
    """
...
def train_a2c(
    ...
    """
    Train an A2C agent on the MinimalEnv environment.
    ...
    if load_model_path and os.path.exists(load_model_path):
        ...
        # Model path format: models/a2c_model_YYYYMMDD_HHMMSS.zip
        model_stem = Path(load_model_path).stem
        if 'a2c_model_' in model_stem:
            model_timestamp = model_stem.replace('a2c_model_', '')
            potential_callback_dir = f"a2c_model_log_{model_timestamp}"
    ...
    if load_model_path and os.path.exists(load_model_path):
        ...
        model = A2C.load(load_model_path, env=env)
...
def load_training_config(config_path=None):
    """
    Load train_a2c keyword arguments from YAML config.

    Args:
        config_path: Optional override path. Defaults to config.yaml next to this file.

    Returns:
        dict: Filtered kwargs to pass into train_a2c.
    """
    path = Path(config_path) if config_path else DEFAULT_CONFIG_PATH
    if not path.exists():
        print(
            f"[config] Config file not found at {path}. Using train_a2c defaults.")
        return {}

    with path.open("r", encoding="utf-8") as f:
        data = yaml.safe_load(f) or {}

    training_cfg = data.get("training", {}).get("a2c", {}) or {}
    filtered_cfg = {k: v for k, v in training_cfg.items()
                    if k in TRAIN_A2C_KWARGS}

    unknown_keys = sorted(set(training_cfg.keys()) - TRAIN_A2C_KWARGS)
    if unknown_keys:
        print(f"[config] Ignoring unsupported train_a2c keys: {unknown_keys}")
