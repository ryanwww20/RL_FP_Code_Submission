...
# Windows uses spawn for multiprocessing which re-imports this module before
# the "__main__" guard runs. Creating worker pools at import time therefore
# triggers the "attempt has been made to start a new process before the current
# process has finished its bootstrapping phase" error. Fall back to a
# single-process direct solver on Windows (or when the environment explicitly
# requests it) to keep things import-safe.
_disable_mp = os.name == "nt" or os.environ.get("SPINS_DISABLE_MP_SOLVER", "").lower() in (
    "1", "true", "yes"
)
if _disable_mp:
