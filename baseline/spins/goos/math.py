...
class Sum(Function):
    ...
    def eval(self, inputs: List[flows.NumericFlow],
        ...
        # Sum sequentially to support mixing scalars and arrays with broadcasting
        arrays = [x.array for x in inputs]
        total = arrays[0]
        for a in arrays[1:]:
            total = total + a
        flow.array = total
...
class Product(Function):
    ...
    def eval(self, inputs: List[flows.NumericFlow]) -> flows.NumericFlow:
        flow = copy.deepcopy(find_dominant_numeric_flow(inputs))
        try:
            # Multiply sequentially to support a mix of scalars and arrays
            arrays = [x.array for x in inputs]
            prod = arrays[0]
            for a in arrays[1:]:
                prod = prod * a
            ...
            # Diagnostic: print shapes and types to help locate incompatible inputs
            shapes = []
            for i, x in enumerate(inputs):
                try:
                    shapes.append((i, type(x.array), getattr(x.array, 'shape', None)))
                except Exception:
                    shapes.append((i, type(x), 'unreadable'))
            msg = f"Product.eval failed computing prod over inputs; input shapes/types: {shapes}. Original error: {e}"
    ...
    def grad(self, input_vals: List[flows.NumericFlow],
        ...
        grads = []
        # Helper to compute product of a list of arrays/scalars sequentially
        def seq_prod(arr_list):
            if not arr_list:
                return 1
            p = arr_list[0]
            for a in arr_list[1:]:
        ...
        for i in range(len(input_val_arr)):
            ...
            prod_before = seq_prod(input_val_arr[:i])
            prod_after = seq_prod(input_val_arr[i + 1:])
